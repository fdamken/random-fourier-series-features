...
def test_constructor():
    RandomFourierSeriesFeaturesKernel(100, RandomFourierSeriesInitializer(10, 0))
...
def test_compute(batch_a, batch_b, samples_p, samples_q, dim):
    rfsf = RandomFourierSeriesFeaturesKernel(100, RandomFourierSeriesInitializer(10, 0))
...
def test_compute_eq(batch_a, batch_b, samples, dim, num_samples):
    rfsf = RandomFourierSeriesFeaturesKernel(num_samples, RandomFourierSeriesInitializer(10, 0))
    p = torch.rand(size=(batch_a, batch_b, samples, dim))
    with gpytorch.settings.lazily_evaluate_kernels(False):
        cov = rfsf(p)
